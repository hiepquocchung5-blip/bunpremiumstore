...
<div class="flex flex-col lg:flex-row gap-6 h-[calc(100vh-140px)] min-h-[600px]">
                ...
                <?php foreach($orders as $order): ?>
                    ...
                    <div class="relative group">
                        <a href="index.php?module=user&page=orders&view_chat=<?php echo $order['id']; ?>" 
                           class="block p-4 rounded-xl transition-all duration-200 border 
                        ...
                        <!-- Invoice Link (Visible if Active) -->
                        <?php if($order['status'] == 'active'): ?>
                            <a href="index.php?module=user&page=invoice&id=<?php echo $order['id']; ?>" 
                               class="absolute bottom-4 right-20 text-[10px] bg-gray-700 hover:bg-gray-600 text-gray-300 px-2 py-1 rounded transition border border-gray-600 z-10" target="_blank">
    ...
    <div class="w-full lg:w-2/3 flex flex-col glass rounded-2xl border border-gray-700/50 shadow-2xl overflow-hidden relative">
        ...
        <div class="p-3 md:p-4 border-b border-gray-700/50 bg-gray-900/50 backdrop-blur-md flex items-center gap-3 shrink-0 z-20">
            ...
            <?php if($active_order['status'] == 'active'): ?>
                <a href="index.php?module=user&page=invoice&id=<?php echo $active_order['id']; ?>" target="_blank" class="bg-gray-800 text-gray-300 hover:text-white hover:bg-gray-700 text-xs px-3 py-1.5 rounded-lg transition border border-gray-600 items-center gap-2 flex">
                    <i class="fas fa-file-invoice"></i> Invoice
                </a>
            <?php endif; ?>
        ...
        <?php if($active_order['delivery_type'] == 'universal' && $active_order['status'] == 'active'): ?>
            <div class="bg-blue-900/10 border-b border-blue-500/30 p-4 shrink-0 relative overflow-hidden">
        ...
        <?php if($active_order['status'] === 'rejected'): ?>
            ...
            <div class="flex-grow flex flex-col items-center justify-center p-8 text-center bg-red-900/5 relative overflow-hidden">
                <div class="absolute inset-0 bg-gradient-to-t from-red-900/20 to-transparent pointer-events-none"></div>
                <div class="w-20 h-20 bg-red-500/10 rounded-full flex items-center justify-center mb-6 border border-red-500/20 shadow-[0_0_30px_rgba(239,68,68,0.15)] relative z-10">
                    <i class="fas fa-times text-4xl text-red-500"></i>
                </div>
                <h3 class="text-2xl font-bold text-white mb-2 relative z-10">Order Rejected</h3>
                <p class="text-gray-400 text-sm max-w-sm mb-8 relative z-10 leading-relaxed">
        ...
        <?php else: ?>
            ...
            <div class="flex-grow overflow-y-auto p-4 space-y-3 bg-black/20 scroll-smooth custom-scrollbar relative" id="chatBox">
                ...
                <?php foreach($chat_messages as $msg): ?>
                    <div class="flex w-full <?php echo $msg['sender_type'] == 'user' ? 'justify-end' : 'justify-start'; ?>">
    ...
    <?php else: ?>
        <div class="hidden lg:flex w-2/3 glass rounded-2xl border border-gray-700/50 items-center justify-center flex-col text-gray-500 p-10 text-center">
            ...
            <p class="max-w-xs mx-auto text-sm">Select an order from the list to view details or chat with support.</p>
        </div>
    <?php endif; ?>
